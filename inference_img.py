...
for i, img in enumerate(img_list):
    if args.img[0].endswith('.exr') and args.img[1].endswith('.exr'):
        cv2.imwrite(f'output/img{i+1}.exr', img[0].cpu().numpy().transpose(1, 2, 0)[:h, :w], 
                    [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_HALF])
    else:
        cv2.imwrite(f'output/img{i+1}.png', (img[0] * 255).byte().cpu().numpy().transpose(1, 2, 0)[:h, :w])
